...
public:
    ...
    ~Rect() { delete lu; delete rl; }

    double area() const {
        return abs((rl->x - lu->x) * (lu->y - rl->y));
    }
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (type == 'R') {
            double x1, x2, y1, y2;
            infile >> x1 >> x2 >> y1 >> y2;
            rects.push_back(new Rect( new Point(x1, y1), new Point(x2, y2) ));
        }
        else if (type == 'C') {
            double xc, yc, r;
            infile >> xc >> yc >> r;
            circles.push_back(new Circle( new Point(xc, yc), r ));
        }
    }

    double xc, xy, radi;
    infile >> xc >> xy >> radi;
    Circle newCircle(new Point(xc, xy), radi);
